...
class PenggunaController extends BaseController
{
    public function index()
    {
        return view('halaman/pengguna/table', [
            'xx'    => (new Pengguna12200651())->get()->getResult(),
    ...
    public function simpan(){
        ...
        if( $this->validasi()){
            $r = (new Pengguna12200651())->insert($data);
            if($r == false){
    ...
    public function edit($id){
        $data = (new Pengguna12200651())->where('id', $id)->first();
    ...
    public function patch(){
        ...
        if( $this->validasiPatch() ){
            $r = (new Pengguna12200651())->update($id, $data);
            if($r == true){
            return redirect()->to('/pengguna-list');
        }else{
    ...
    public function delete(){
        $id = $this->request->getPost('id');
        $r = (new Pengguna12200651())->delete($id);
